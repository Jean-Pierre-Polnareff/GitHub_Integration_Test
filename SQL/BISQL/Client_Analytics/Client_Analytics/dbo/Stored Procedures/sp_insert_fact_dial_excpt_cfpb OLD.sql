...
CREATE PROCEDURE [dbo].[sp_insert_fact_dial_excpt_cfpb OLD]
...
----------------------------------------- #calls for last 31 days ----------------------------------------------------------------------------------
	...
	SELECT 
			fct.KeyCustomerCall
			, cust.ClientId
			, case when dss.SourceSystem='Amex Latitude' then isnull(fct.Consumer_ID,fct.KeyCustomer)
			       else fct.KeyCustomer
				   end as KeyCustomer
			, cust.CustomerId
			, fct.KeyEmployee
			, dss.KeySourceSystem
	...
	--7th previous call start time since limit of 7 calls in 7 days.  If 7th call in less than 7 days, exception.
			, LAG(fct.CallStartTime,7) OVER(PARTITION BY case when dss.SourceSystem='Amex Latitude' 
			                                                  then isnull(fct.Consumer_ID,fct.KeyCustomer)
			                                                  else fct.KeyCustomer
				                                              end 
	...
	WHERE dt.CalendarDate > isnull(@StartDateTime,GETDATE())-31
		  ...
		  AND dcl.clientid NOT IN('SNBCEP','DCMYSP','DCABBP','DCABYP','DCAD2P','DCADSP','DCAFDP','DCAFLP','DCAFSP','DCAKDP',
                                  'DCAOMP','DCAPDP','DCAVAP','DCCSSP','DCMYSP','SNBCE1','ATTMOB')
