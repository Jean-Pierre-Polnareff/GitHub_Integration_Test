...
GO
/****** Object:  StoredProcedure [dbo].[sp_insert_fact_letter_excpt_compliance_check]    Script Date: 7/8/2024 2:12:53 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	DROP TABLE IF EXISTS #temp_customer_fcc_hist  
	
	SELECT DISTINCT fcc.KeyCustomer 
	INTO #temp_customer_fcc_hist 
	...
	FROM #temp_customer t    
		left join #temp_customer_fcc_hist hist on hist.KeyCustomer = t.KeyCustomer
		JOIN DW_MSTR_DM.dbo.FactCustomerCall fcc WITH (NOLOCK) ON fcc.KeyCustomer = t.KeyCustomer 
		JOIN DW_MSTR_DM.dbo.DimDialerResult r with (nolock) on r.KeyDialerResult = fcc.KeyDialerResult 
	where hist.KeyCustomer is null 
		and (fcc.IsRPC = 1                                                       
			or r.Dialer_Result like '%Left Message%') 
	...
	INTO #temp_cust 
	FROM #temp_customer_firstevercontact t WITH (NOLOCK) 
		JOIN dw_mstr_dm.dbo.DimCustomer c with (nolock) on c.keycustomer = t.keycustomer 
	...
	SELECT ltr_expt_id = 3,
		...
		Is_PIF = CASE WHEN c.InitialBalance = c.PaidOnAccountAmt THEN 1 ELSE 0 END, 
		Is_SIF = CASE WHEN c.SIFAmt IS NOT NULL then 1 
					WHEN c.SIFAmt > 0  
					THEN 1 ELSE 0 END,
		c.LastPaymentDate, 
