...
GO
/****** Object:  StoredProcedure [dbo].[usp_RPT_email_guid_INS]    Script Date: 10/24/2024 7:28:31 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[usp_RPT_email_guid_INS]
...
SET NOCOUNT ON;                                           
	...
	INTO #sends
	FROM dw_mstr_dm.[dbo].[Radius_EmailReportData] emrd WITH (NOLOCK, forceseek)
	...
	group by cast(s.EventDate as DATE)
	 ...
	 FROM #inserts i with (nolock) 
		LEFT JOIN CLIENT_ANALYTICS.dbo.RPT_email_guid g with (nolock, forceseek) on g.GUID = i.GUID 
