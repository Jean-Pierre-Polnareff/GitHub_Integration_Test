...
-- =============================================
--Amod Ramugade 2023-08-31 Adding a line of code to call another proc [CLIENT_ANALYTICS].[dbo].[sp_insert_RPT_SMS_Optout_detail] 
--                         which is used to identify and insert SMS OptOuts into [CLIENT_ANALYTICS].[dbo].[RPT_SMS_Optout_detail]
-- =============================================
 
CREATE PROCEDURE [dbo].[sp_insert_RPT_SMS_Optout_Excpt]
...
----sent SMS data with Livevox result SMS MT Delivered','SMS MT Failed for exception rule_1----
	drop table if exists #RadiusCall

	select rc.Session_Id, rc.Call_Date, rc.Livevox_Result  
			, rc.Answer_Type 
			, rc.Service_Name 
	into #RadiusCall 
	from DW_MSTR_DM.dbo.RadiusCall rc WITH (NOLOCK)
	where rc.Call_Date >= ISNULL(null, GETDATE()) - 31      

	create index #ix_#RadiusCall on #radiuscall ([Session_Id], Call_Date) include (Livevox_Result, Answer_Type, Service_Name) 
	...
	FROM DW_MSTR_DM.dbo.FactCustomerCall fct (NOLOCK)
		join #RadiusCall rc ON fct.SessionId = rc.Session_Id  
														AND rc.Call_Date >= ISNULL(@startdatetime, GETDATE()) - 31   
														AND fct.KeyDate_CallDate >= CONVERT(VARCHAR,CAST(ISNULL(@startdatetime, GETDATE()) - 31 as date),112)
	...
	WHERE (rc.livevox_result like 'SMS%'  
	      ...
	      --and cast(rc.question_9 as int) > 0			 				--per TedM 6/29, templateid>0 indicates  send
          AND (rc.Call_Date >= ISNULL(@startdatetime, GETDATE()) - 31 
...
FROM #temp_sms t 
	...
	JOIN DW_MSTR_DM.dbo.RadiusPhone RP (NOLOCK) on cust.KeyCustomer=RP.KeyCustomer  
											AND (CAST(rp.PhoneNumber AS VARCHAR) = CAST(t.DialedPhoneNumber AS VARCHAR) 
												OR '1' + CAST(rp.PhoneNumber AS VARCHAR) = CAST(t.DialedPhoneNumber AS VARCHAR) 
												OR CAST(rp.PhoneNumber AS VARCHAR) = '1' + CAST(t.DialedPhoneNumber AS VARCHAR))  
...
------------SMS_optout exceptions - SMS sent after consumer requested to stop to text------
	...
	WHERE ispoststopsent = 1 
		...
		AND DATEDIFF(MINUTE,previous_callstarttime, CallStartTime) >  60 
			...
			AND NOT (s.CustomerId in ('11644','1224851','1687','1028999','14892599') and s.KeySourceSystem not in (0,4)) 
...
--------Exceptions SMS sent to accounts within 7 days - list of services eligible---------
	...
	from #SMS s  
		left join (
					select cl. clientid, SourceSystem, e.threshold  
					from DW_MSTR_DM.dbo.DimClient cl 
						join [CLIENT_ANALYTICS].[dbo].[dim_digital_sms_excpt_thresholds] e on e.ClientParent = cl.ClientParent 
				) t on t.ClientId = s.ClientId 
	WHERE (DATEDIFF(DAY,s.previous_callstarttime, s.CallStartTime) < CASE 
																	WHEN t.threshold IS NULL 
																	THEN CASE 
																		WHEN s.ClientStreamId LIKE '%Harvest%' 
																		OR s.Service_Name in ('Decorah_LabCorp HTI','Decorah_LabCorp HTI_CTA3','Jacksonville_LabCorp_HTI','Jacksonville_LabCorp_HTI_CTA3') 

																		--As per request from Brett and internal discussion on 2/18/2025, adding clients to the criteria,
																		    OR s.ClientStreamId IN ( 'IMPRAGROUP','IMEVERSRCE','IMZOTEC','IMCLINK','IMBRADFORD','IMALLY','IMNAVIENT','IMCOX','IMVELOCITY')  ------- Clients ->> DEC PRA, MUM PRA, Eversource, MW Radiology, Century Link, Bradford, Ally, Navient, Cox, Velocity
		                                                                    OR s.ClientParent IN ('LABCORPP','LABCORPT')                                                                                    ------- Clients ->> DEC LabCorp, MBJ LabCorp
		                                                                    OR s.ClientId IN ('ATTDJ0','ATTIRU','ATTUCR','ATTUCE','ATTUBO','ATTUB3','ATTUBO','ATTUC3','ATTUHO')                             ------- Clients ->> DTV Early Out,	Mobility Tertiary Consumer,	Uverse	
																		THEN 4 
																		ELSE 7 
																		END  
																	ELSE t.threshold END  
...
--------Exceptions SMS sent to accounts within 4 days---------
	
	select   Digital_excpt_id = 4
           , s.ClientId
		   , s.KeyCustomer
		   , s.CustomerId
		   , s.KeySourceSystem
		   ...
		   , [SMS_Stopped_flag] = NULL	   		   		   	
	from #SMS s
	where (DATEDIFF(day,s.previous_callstarttime, s.CallStartTime) between 1 and 4)
		and s.IsPreviousDelivered = 1 
		and s.IsPreviousStop = 0 
		and s.IsPostStopSent = 0 
		and s.Livevox_Result = 'SMS MT Delivered'
		...
		and 
		(
		   s.ClientStreamId IN ( 'IMPRAGROUP','IMEVERSRCE','IMZOTEC','IMCLINK','IMBRADFORD','IMALLY','IMNAVIENT','IMCOX','IMVELOCITY')  ------- Clients ->> DEC PRA, MUM PRA, Eversource, MW Radiology, Century Link, Bradford, Ally, Navient, Cox, Velocity
		OR s.ClientParent IN ('LABCORPP','LABCORPT')                                                                                    ------- Clients ->> DEC LabCorp, MBJ LabCorp
		OR s.ClientId IN ('ATTDJ0','ATTIRU','ATTUCR','ATTUCE','ATTUBO','ATTUB3','ATTUBO','ATTUC3','ATTUHO')                             ------- Clients ->> DTV Early Out,	Mobility Tertiary Consumer,	Uverse
		)
/*
...
-------------------------------------------identify and insert SMS optouts for last 31 days into client_analytics.dbo.RPT_SMS_Optout_detail-------------------------------------------
EXEC sp_insert_RPT_SMS_Optout_detail;