...
GO
/****** Object:  StoredProcedure [dbo].[sp_insert_fact_dial_excpt_compliance_check]    Script Date: 12/18/2024 1:53:57 PM ******/
SET ANSI_NULLS ON
GO
...
END AS Localized_Time
	...
	FROM DW_MSTR_DM.dbo.FactCustomerCall fct WITH (NOLOCK)
			  inner join
		 DW_MSTR_DM.dbo.RadiusCall rc WITH (nolock) on fct.SessionId=rc.Session_Id and rc.Call_Date >= isnull(@StartDateTime,GETDATE()) -31
			  inner join
...
----into #firsteverrestrictedcalls
		...
		FROM DW_MSTR_DM.dbo.FactCustomerCall fct WITH (NOLOCK)
			  inner join
		 DW_MSTR_DM.dbo.RadiusCall rc WITH (nolock) on fct.SessionId=rc.Session_Id 
...
*/	
	...
	UNION 
		...
		FROM #calls c 	
		WHERE CAST(CallStartTime AS DATE) > CancelDate 
