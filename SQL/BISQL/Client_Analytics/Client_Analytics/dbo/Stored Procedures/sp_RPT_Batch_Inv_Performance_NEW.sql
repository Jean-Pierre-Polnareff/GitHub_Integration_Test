...
create   PROCEDURE [dbo].[sp_RPT_Batch_Inv_Performance_NEW]
	 
	@StartDateTime DATETIME = NULL
	
AS
/* 
Object: dbo.sp_RPT_Batch_Inv_Performance_NEW
...
BEGIN
	...
	declare 
		@in_dt	date = (select isnull(max(cast(insertdate as date)),'1900-01-01')  
		                from CLIENT_ANALYTICS.dbo.RPT_Batch_Inv_Performance_NEW);  
	...
	--Delete from RPT_Batch_Inv_Performance_NEW where matching list_mo/keysourcesystem/clientid
	DELETE bip
	FROM CLIENT_ANALYTICS.dbo.RPT_Batch_Inv_Performance_NEW bip
	       join
	...
	--Reprocess summaries and insert to RPT_Batch_Inv_Performance_NEW for each list_mo/keysourcesystem/clientid

	--Non-FACS
			INSERT INTO CLIENT_ANALYTICS.dbo.RPT_Batch_Inv_Performance_NEW
						   (
							  batch_month
							  , KeySourceSystem
							  , ClientID
			...
			SELECT dd.monthdate as batch_month
					...
					, Charge_Off_Ind = CASE WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= 0
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < .5
             THEN 'A - <6mos'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= .5
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < 1
             THEN 'B -6mos-12mos'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= 1
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < 1.5
             THEN 'C -12mos-18mos'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= 1.5
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < 2
             THEN 'D -18mos-24mos'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= 2
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < 3
             THEN 'E -2yr-3yrs'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= 3
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < 4
             THEN 'F -3yr-4yrs'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= 4
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < 5
             THEN 'G -4yr-5yrs'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= 5
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < 6
             THEN 'H -5yr-6yrs'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= 6
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < 7
             THEN 'I -6yr-7yrs'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= 7
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < 8
             THEN 'J -7yr-8yrs'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= 8
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < 9
             THEN 'K -8yr-9yrs'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= 9
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < 10
             THEN 'L -9yr-10yrs'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= 10
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < 11
             THEN 'M -10yr-11yrs'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= 11
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < 12
             THEN 'N -11yr-12yrs'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= 12
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < 13
             THEN 'O -12yr-13yrs'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= 13
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < 14
             THEN 'P -13yr-14yrs'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= 14
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < 15
             THEN 'Q -14yr-15yrs'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= 15
             THEN 'R -15+ years'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < 0
             THEN 'S -Missing Info'
        END
		, Balance_Ranges = CASE 
				WHEN dcu.INITIALBALANCE >0 AND dcu.INITIALBALANCE <500 THEN 'A-0-$499' 
				WHEN dcu.INITIALBALANCE >=500 AND dcu.INITIALBALANCE <1000 THEN 'B-$500-$999'
				WHEN dcu.INITIALBALANCE >=1000 AND dcu.INITIALBALANCE <2500 THEN 'C-$1000-$2499'
				WHEN dcu.INITIALBALANCE >=2500 AND dcu.INITIALBALANCE <4999 THEN 'D-$2500-$4999'
				WHEN dcu.INITIALBALANCE >=5000 THEN 'E-$5000+'
				END
			from dw_mstr_dm.dbo.DimCustomer dcu (nolock)
					join
				 DW_MSTR_DM.dbo.DimDate dd (nolock) on dcu.ListDate=dd.CalendarDate
			...
			GROUP BY dd.monthdate
					...
					, dcu.clientid
					, dcu.customerstate
					, CASE WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= 0
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < .5
             THEN 'A - <6mos'
             WHEN DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 >= .5
                  AND DATEDIFF(DAY,ISNULL(Cast(dcu.CHARGEOFFDATE as date),'1/1/2040'),ISNULL(Cast(dcu.LISTDATE as date),'1/1/2050')) / 360.00 < 1
