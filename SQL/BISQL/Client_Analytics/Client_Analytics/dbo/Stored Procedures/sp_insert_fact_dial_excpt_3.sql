...
*/
    ...
    -- Config joins
    LEFT JOIN CLIENT_ANALYTICS.dbo.Dial_Excpt_PhoneType pt WITH (NOLOCK)
        ON fct.PhonePosition = pt.PhonePosition AND pt.Is_FACS = 0

    LEFT JOIN CLIENT_ANALYTICS.dbo.Dial_Excpt_1stParty_Services_Exclude_or_Suppress css WITH (NOLOCK)
        ON rc.service_id = css.service_id AND css.Is_Suppress = 1
		   AND css.Is_CFPB = 0 AND css.Is_Compliance = 0
    ...
    LEFT JOIN CLIENT_ANALYTICS.dbo.Dial_Excpt_ClientID_Exclude_or_Suppress ccsv WITH (NOLOCK)
        ON dcl.ClientId = ccsv.client_id AND ccsv.Is_Suppress = 1
		   AND ccsv.Is_CFPB = 0 AND ccsv.Is_Compliance = 0

    LEFT JOIN CLIENT_ANALYTICS.dbo.Dial_Excpt_State_Rules csr WITH (NOLOCK)
	...
	IF OBJECT_ID('tempdb..#exceptions') IS NOT NULL
		...
		FROM #call_seq cs1
			 ...
			 #call_seq cs2 ON cs1.keycustomer=cs2.keycustomer
				...
				LEFT JOIN
             CLIENT_ANALYTICS.dbo.Dial_Excpt_ClientID_Exclude_or_Suppress cvce WITH (NOLOCK) 
			 ON cs1.ClientId = cvce.client_id AND cvce.Is_Exclude = 1 
			 AND cvce.Is_CFPB = 0 AND cvce.Is_Compliance = 0
			----1stparty excluded per BrettL
				LEFT JOIN
			 CLIENT_ANALYTICS.dbo.Dial_Excpt_1stParty_Services_Exclude_or_Suppress dvs WITH (NOLOCK) 
			 ON cs1.Service_Id = dvs.Service_Id AND dvs.Is_Exclude = 1 
			 AND dvs.Is_CFPB = 0 AND dvs.Is_Compliance = 0
