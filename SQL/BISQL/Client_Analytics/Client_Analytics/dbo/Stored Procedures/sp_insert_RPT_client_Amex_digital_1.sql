...
CREATE PROCEDURE [dbo].[sp_insert_RPT_client_Amex_digital]
...
BEGIN
	...
	DROP TABLE IF EXISTS #inv
		...
		FROM DW_MSTR_DM.dbo.DimCustomer dcu WITH (NOLOCK)
				JOIN
			 DW_MSTR_DM.dbo.DimClient dcl WITH (NOLOCK) ON dcu.ClientId=dcl.ClientId
		...
		FROM (
				...
				FROM #ic_ld ild
					 ...
					 #cael cael ON ilr.AccountID=cael.Number AND(ilr.EmailLetterRequestId=cael.UID 
						...
						JOIN
					 DW_MSTR_DM.dbo.DimCustomer dcu WITH (NOLOCK) ON cael.Number=dcu.CustomerId
																AND dcu.SourceSystem LIKE 'Amex%'
						join
	...
	--SMS
		...
		FROM (	
				...
				FROM DW_MSTR_DM.dbo.RadiusCall rc WITH (NOLOCK)
						JOIN
					 DW_MSTR_DM.dbo.FactCustomerCall fcc WITH (NOLOCK) ON rc.Session_Id=fcc.SessionId
						JOIN
					 DW_MSTR_DM.dbo.DimCustomer dcu WITH (NOLOCK) ON fcc.KeyCustomer=dcu.KeyCustomer
						JOIN
					 DW_MSTR_DM.dbo.DimClient dcl WITH (NOLOCK) on dcu.ClientId=dcl.ClientId
						LEFT JOIN
					 #dec d ON dcu.CustomerId = d.Number
				...
				FROM #fsms f
						JOIN
					 DW_MSTR_DM.dbo.DimCustomer dcu WITH (NOLOCK) ON f.Number=dcu.CustomerId
													   AND dcu.SourceSystem LIKE 'Amex%'
						JOIN
					 #client_lu cl ON dcu.ClientId=cl.ClientCode
						LEFT JOIN
	...
	--letters
		...
		FROM (	
				...
				FROM DW_MSTR_DM.dbo.FactCustomerLetter fcl WITH (NOLOCK)
						JOIN
					 DW_MSTR_DM.dbo.DimDate dd WITH (NOLOCK) ON fcl.KeyDate_MailDate=dd.KeyDate
						join
					 DW_MSTR_DM.dbo.DimCustomer dcu WITH (NOLOCK) ON fcl.KeyCustomer=dcu.KeyCustomer
						JOIN
					 DW_MSTR_DM.dbo.DimClient dcl WITH (NOLOCK) on dcu.ClientId=dcl.ClientId
						LEFT JOIN
					 #dec d ON dcu.CustomerId=d.Number
				...
				FROM #ic_ld icl
					 ...
					 #ic_lr ilr ON icl.LatLetterRequestId=ilr.LetterRequestID
						join
					 DW_MSTR_DM.dbo.DimCustomer dcu WITH (NOLOCK) ON ilr.AccountID=dcu.CustomerId
																AND dcu.SourceSystem LIKE 'Amex%'
						JOIN
					 DW_MSTR_DM.dbo.DimClient dcl WITH (NOLOCK) on dcu.ClientId=dcl.ClientId
						JOIN
	...
	--calls
		...
		FROM DW_MSTR_DM.dbo.FactCustomerCall fcc WITH (NOLOCK)
				JOIN
			 DW_MSTR_DM.dbo.RadiusCall rc WITH (NOLOCK) ON fcc.SessionId=rc.Session_Id
				JOIN
			 DW_MSTR_DM.dbo.DimDate dd WITH (NOLOCK) ON fcc.KeyDate_CallDate=dd.KeyDate
				join
			 DW_MSTR_DM.dbo.DimCustomer dcu WITH (NOLOCK) ON fcc.KeyCustomer=dcu.KeyCustomer
				JOIN
			 DW_MSTR_DM.dbo.DimClient dcl WITH (NOLOCK) on dcu.ClientId=dcl.ClientId
	...
	--payments
		...
		FROM DW_MSTR_DM.dbo.FactCustomerPayment fcp WITH (NOLOCK)
				JOIN
			 DW_MSTR_DM.dbo.DimDate dd WITH (NOLOCK) ON fcp.KeyDate_PaymentDate=dd.KeyDate
				join
			 DW_MSTR_DM.dbo.DimCustomer dcu WITH (NOLOCK) ON fcp.KeyCustomer=dcu.KeyCustomer
				JOIN
			 DW_MSTR_DM.dbo.DimClient dcl WITH (NOLOCK) on dcu.ClientId=dcl.ClientId
				LEFT JOIN
			 #dec d ON dcu.CustomerId=d.Number
...
--  changed above to only grab payments for the single payment date
	...
	--first rpcs for payers
		...
		FROM #pay0 p
			   LEFT JOIN
			 DW_MSTR_DM.dbo.FactCustomerCall fcc WITH (NOLOCK) ON p.KeyCustomer=fcc.KeyCustomer
	...
	--pay final
		...
		FROM #firstrpc f
				LEFT JOIN
			 DW_MSTR_DM.dbo.RadiusCall rc WITH (NOLOCK) ON f.SessionId=rc.Session_Id
		GROUP BY f.rpt_year
			   , f.rpt_month
			   , f.rpt_date
			   , f.StandardParentCode
	...
	DROP TABLE IF EXISTS #web
		...
		INTO #web
		FROM DW_MSTR_DM.[dbo].[Web_Waterfall_Metrics_Data_Client] wm WITH (NOLOCK)
			   join
		   DW_MSTR_DM.dbo.DimClient dcl WITH (NOLOCK) on wm.ClientId=dcl.ClientId
			   LEFT JOIN
		   #dec d ON TRY_CAST(SUBSTRING(wm.ReferenceNumber,5,8) AS INT) = d.Number 
			   LEFT JOIN
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[sp_insert_RPT_client_Amex_digital] TO [corp\ravijaykumar]
    AS [dbo];
...
GO
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[sp_insert_RPT_client_Amex_digital] TO [CORP\mhuang]
    AS [dbo];
...
GO
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[sp_insert_RPT_client_Amex_digital] TO [CORP\aramugade]
    AS [dbo];
...
GO
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[sp_insert_RPT_client_Amex_digital] TO [CORP\aughodake]
    AS [dbo];
