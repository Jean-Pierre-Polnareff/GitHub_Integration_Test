...
CREATE PROCEDURE [dbo].[sp_insert_fact_dial_excpt_cfpb_other]
...
--			, rc.Livevox_Result
	--7th previous call start time since limit of 7 calls in 7 days.  If 7th call in less than 7 days, exception.
			, LAG(rc.Call_Connect_Time_CT,7) OVER(PARTITION BY rc.Account_Number ORDER BY rc.Call_Connect_Time_CT, rc.Session_Id) AS callstarttime_lag7
	--isrpc_lag1 indicates previous call was an RPC
			, LAG(rc.Is_RPC,1) OVER(PARTITION BY rc.Account_Number ORDER BY rc.Call_Connect_Time_CT, rc.Session_Id) AS isrpc_lag1
	--callstarttime_lag1 for calculating whether call following RPC is within 7 days
			, LAG(rc.Call_Connect_Time_CT,1) OVER(PARTITION BY rc.Account_Number ORDER BY rc.Call_Connect_Time_CT, rc.Session_Id) AS callstarttime_lag1
	INTO #calls 
