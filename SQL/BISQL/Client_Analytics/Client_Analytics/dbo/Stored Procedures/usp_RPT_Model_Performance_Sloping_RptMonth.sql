...
ALTER   PROCEDURE [dbo].[usp_RPT_Model_Performance_Sloping_RptMonth]
...
Mike Campbell 	08/31/2021	Initial creation 
Amod Ramugade   10/10/2023  Addition of ClientId, ClientParent, ClientStream, ClientStreamId
*/
...
TRUNCATE TABLE CLIENT_ANALYTICS.dbo.RPT_Model_Performance_Sloping_RptMonth;

	--samp from 9/1/21 thru 6 months ago
	 ...
	 (
	  SELECT dm.KeyModel
		...
		WHERE mp.list_month<=DATEADD(MONTH,-6,GETDATE())
			  AND mp.list_month>='9/1/20'
			  AND LEN(mp.clientparent)>0
			  AND dm.ModelLongDescription<>'Utility Segmentation'
	 ...
	 (
	  SELECT dm.KeyModel
		...
		FROM --[CLIENT_ANALYTICS].[dbo].[RPT_Model_Performance_listmo] mp
			 ...
			 DW_MSTR_DM.dbo.DimClient dcl WITH (NOLOCK)  on mp.ClientId=dcl.ClientId
                join
			 DW_MSTR_DM.dbo.DimSourceSystem dss on dcl.SourceSystem=dss.SourceSystem2
			                                       and mp.KeySourceSystem=dss.KeySourceSystem
		WHERE mp.rpt_month>=DATEADD(MONTH,-3,GETDATE())
			  AND LEN(mp.clientparent)>0
			  AND dm.ModelLongDescription<>'Utility Segmentation'
	 ...
	 --leaving some commented cols below for future quick reference on calculation of predicted_call_pct if needed
	  INSERT INTO CLIENT_ANALYTICS.dbo.RPT_Model_Performance_Sloping_RptMonth
	 (
	 [KeyModel]
	...
	--		, yt.yield_idx_tot
			...
			, case when yt.yield_idx_tot is null or yt.yield_idx_tot = 0 then 0 else (case when tv.yield is null or tv.yield = 0 then 0 else sv.yield / tv.yield end) / yt.yield_idx_tot end AS predicted_call_pct
			, cm.SourceSystem
	 FROM currmo cm
			LEFT JOIN
