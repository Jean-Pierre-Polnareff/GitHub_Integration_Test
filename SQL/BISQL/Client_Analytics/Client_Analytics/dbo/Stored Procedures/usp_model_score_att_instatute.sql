...
CREATE   PROCEDURE [dbo].usp_model_score_att_instatute (@model VARCHAR(100),@q NVARCHAR(MAX))
as 
DECLARE @rx_model VARBINARY(MAX) = (SELECT model FROM CLIENT_ANALYTICS.dbo.xgb_models WHERE model_name = @model);
    ...
    EXECUTE sp_execute_external_script
      ...
      , @script = N'
		    ...
		    test=InputDataSet;
						x_test=data.frame(select(test,c(has_previous_acct_telecom,	
														age_acct_at_listdate_r,
														has_previous_acct_nontelecom, 
														state_level,
														clientidgrp_level
														)));
