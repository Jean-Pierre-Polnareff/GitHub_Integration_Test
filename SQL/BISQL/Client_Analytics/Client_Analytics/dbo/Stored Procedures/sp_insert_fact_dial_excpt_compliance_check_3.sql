...
CREATE PROCEDURE [dbo].[sp_insert_fact_dial_excpt_compliance_check]
...
AND dlr_excpt_id  IN (14,15,16,17,18,19,20,21,32)  
...
WHERE 
dde.dlr_expt_id  IN (14,15,16,17,18,19,20,21,32) 
AND 
dss.KeySourceSystem not in (0,4)  
...
SELECT 
			...
			,LAG(fct.CallStartTime,3) OVER(PARTITION BY cust.KeyCustomer ORDER BY fct.CallStartTime, fct.SessionId) AS callstarttime_lag3
			,t.State_Name
			,rc.Service_Name
	...
	FROM #temp_fcc fct 
		 ...
		 #temp_rc rc  on fct.SessionId=rc.Session_Id 
			  inner join
		 DW_MSTR_DM.dbo.DimSourceSystem dss WITH (nolock) on fct.KeySourceSystem=dss.KeySourceSystem
			  inner join 
		 DW_MSTR_DM.dbo.DimCustomer cust WITH (NOLOCK)ON fct.KeyCustomer = cust.KeyCustomer and cust.StatusCode<>'DW_deactivate'
			  inner join
		 DW_MSTR_DM.dbo.DimDate dt WITH (NOLOCK) ON fct.KeyDate_CallDate = dt.KeyDate
		--	  LEFT outer JOIN 
		-- DW_MSTR_DM.dbo.RadiusPhone RP WITH (NOLOCK) on cust.KeyCustomer=RP.KeyCustomer and fct.DialedPhoneNumber=RP.PhoneNumber
		      left outer join
		 DW_MSTR_DM.dbo.DimEmployee de WITH (NOLOCK) on fct.KeyEmployee=de.KeyEmployee
		      left outer join
		 DW_MSTR_DM.dbo.DimClient dcl WITH (NOLOCK) on cust.ClientId=dcl.ClientId and cust.SourceSystem=dcl.SourceSystem
		 LEFT OUTER JOIN 
		 DW_MSTR_DM.dbo.TimeZoneByState T (NOLOCK) ON cust.CustomerState = T.State_Abbr
		 left join 
		 [CLIENT_ANALYTICS].[dbo].[vw_Amex_ClientCodes_LookupTable] (nolock) lup  on lup.ClientCode= cust.ClientId and cust.SourceSystem= 'AMEX Latitude'
		 left join 
		 [CLIENT_ANALYTICS].[dbo].[Dial_Excpt_ClientID_Exclude_or_Suppress] cvce (nolock) on dcl.ClientId = cvce.client_id 
		 AND cvce.Is_Exclude = 1 AND cvce.Is_Compliance = 1
...
--7/5/22  -  TedM requests all services containing "HTI" (sms) to be suppressed 
		--and rc.Service_Name not like '%HTI%' AND rc.Service_Name <> 'IDL Artiva SMS'
        --AND rc.livevox_result  NOT LIKE 'SMS%' AND rc.livevox_result NOT LIKE '%Text%' 
		--AND dcl.ClientId NOT LIKE 'DC%P'       ---suppressing client DentalCareAP
		AND dcl.ClientStreamId <> 'IMDENTCARE'   ---suppressing client DentalCareAP
	    --AND dcl.ClientId NOT IN ('SNBCEP', 'EMPBSP','EMPCMF','EMPIFP', 'ATRAB1')
...
----into #firsteverrestrictedcalls
		...
		FROM #temp_fcc fct 
		 ...
		 DW_MSTR_DM.dbo.DimSourceSystem dss (nolock) on fct.KeySourceSystem=dss.KeySourceSystem
			  inner join 
		 DW_MSTR_DM.dbo.DimCustomer cust WITH (NOLOCK) ON fct.KeyCustomer = cust.KeyCustomer 
		 and cust.StatusCode<>'DW_deactivate'
			  inner join
		 DW_MSTR_DM.dbo.DimDate dt WITH (NOLOCK) ON fct.KeyDate_CallDate = dt.KeyDate
		 left join 
		 [CLIENT_ANALYTICS].[dbo].[vw_Amex_ClientCodes_LookupTable] lup with (nolock) 
         on lup.ClientCode= cust.ClientId
         and cust.SourceSystem= 'AMEX Latitude' 
		 LEFT OUTER JOIN 
		 CLIENT_ANALYTICS.dbo.build_first_ever_restricted_Calls w WITH (NOLOCK)  
...
----into #firsteverrestrictedDNCcalls
		...
		LEFT outer JOIN 
		 DW_MSTR_DM.dbo.RadiusPhone RP WITH (NOLOCK) on cust.KeyCustomer=RP.KeyCustomer 
		 ...
		 */
		 left join [CLIENT_ANALYTICS].[dbo].[vw_Amex_ClientCodes_LookupTable] lup WITH (nolock) 
         on lup.ClientCode= cust.ClientId
         and cust.SourceSystem= 'AMEX Latitude'
		 LEFT OUTER JOIN 
		 CLIENT_ANALYTICS.dbo.build_first_ever_restricted_Calls w WITH (NOLOCK)  
	...
	------dlr_expt_id=14:  not more than 3 attempts per day
		...
		FROM #calls c
			----1stparty excluded per BrettL
				LEFT JOIN
		CLIENT_ANALYTICS.dbo.Dial_Excpt_1stParty_Services_Exclude_or_Suppress dvs ON c.[Service_Name] = dvs.[Service_Name] AND dvs.Is_Exclude = 1 AND dvs.Is_Compliance = 1
		WHERE 
		 CAST(c.callstarttime_lag3 AS DATE) = CAST(c.CallStartTime AS DATE)
----As per Brett on 12/5/22, excluded below service_name
AND c.[Service_Name] IS NULL

AND c.EmployeeId <> 'TRodrigues'	

AND c.DialedPhoneNumber NOT IN
(
 '8007424932'
,'3157244022'
,'8007630555'
...
,'9035684211'
    ...
    ---dlr_expt_id=15:  Post-DNC 0 attempts for lifetime	
        ...
        ,SourceSystem
        ,PhoneNumber
        FROM CLIENT_ANALYTICS.dbo.build_first_ever_restricted_DNC_Calls WITH (NOLOCK)
    ...
    ----dlr_expt_id=16: Post Attorney Handling/Representation 0 attempts for lifetime	
        ...
        ,CallStartTime
        ,SourceSystem
        FROM CLIENT_ANALYTICS.dbo.build_first_ever_restricted_Calls WITH (NOLOCK)
...
*/	
	...
	UNION 
		...
		FROM #calls c 	
		WHERE CAST(c.CallStartTime AS DATE) > c.CancelDate AND c.CancelDate <> '1900-01-01' 
...
(
	...
	FROM #exceptions exc
		LEFT JOIN
		 CLIENT_ANALYTICS.dbo.fact_dial_excpt fde WITH (NOLOCK) 
