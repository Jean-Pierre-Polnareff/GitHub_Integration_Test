...
GO
/****** Object:  StoredProcedure [dbo].[sp_insert_fact_dial_excpt_facs]    Script Date: 7/5/2022 4:03:00 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER  PROCEDURE [dbo].[sp_insert_fact_dial_excpt_FACS]
	...
	IF OBJECT_ID('tempdb..#call_seq') IS NOT NULL
		...
		SELECT *
			   , CASE WHEN custstate_ma=1 AND phonetype_home=1
					  THEN ROW_NUMBER() OVER(PARTITION BY customerid, custstate_ma, phonetype_home ORDER BY callstarttime)
					  END AS rank_ma_home
			   , CASE WHEN custstate_ma=1 AND phonetype_poe=1
					  THEN ROW_NUMBER() OVER(PARTITION BY customerid, custstate_ma, phonetype_poe ORDER BY callstarttime)
					  END AS rank_ma_poe
			   , CASE WHEN custstate_ma=1 AND phonetype_other=1
					  THEN ROW_NUMBER() OVER(PARTITION BY customerid, custstate_ma, phonetype_other ORDER BY callstarttime)
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[sp_insert_fact_dial_excpt_FACS] TO [CORP\aramugade]
    AS [dbo];
