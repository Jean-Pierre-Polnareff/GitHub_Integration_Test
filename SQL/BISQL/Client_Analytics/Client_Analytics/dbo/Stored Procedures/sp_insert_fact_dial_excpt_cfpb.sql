...
GO
/****** Object:  StoredProcedure [dbo].[sp_insert_fact_dial_excpt_cfpb]    Script Date: 7/5/2022 4:03:00 PM ******/
SET ANSI_NULLS ON
GO
...
----------------------------------------- #calls for last 31 days ----------------------------------------------------------------------------------
	...
	--12/8 - Bob requests removing Amex calls not in set of clientids described by Bilal/Susheel. Set flag to track, then alert below
	--12/10 - additional codes added by Bilal/Susheel to list below
	...
	FROM DW_MSTR_DM.dbo.FactCustomerCall fct (NOLOCK)
		 ...
		 DW_MSTR_DM.dbo.RadiusCall rc (nolock) on fct.SessionId=rc.Session_Id 
		                                          ...
		                                          and rc.Call_Date < isnull(@StartDateTime,GETDATE())
			  inner join
	...
	WHERE dt.CalendarDate > isnull(@StartDateTime,GETDATE())-31
	      and dt.CalendarDate < isnull(@StartDateTime,GETDATE())
	      AND fct.IsOutbound=1
          --11/11/21 - TedM requests no limits on Amex with following producttype codes
		  AND ISNULL(dcp.ProductType,'') NOT IN('EX','SB','SM','SR','AB','HC','HP','BT','CB','CC','CP','CR','DV')
          --11/11/21 - TedM requests no limits on Amex with following dispositions
		  --11/18/21 - TedM adds AGENT - CUST 8 to veldos exclusion
		  --11/30/21 - TedM adds:  'Busy','Fax','Invalid Phone Number'
		  --1/3/22 - Brett adds:  'AGENT - CUST RPC PTP 6'
		  ...
		  --12/7/21 - Bob/TedM TedM request no limits on Amex for service_name=Payment Verification Manual
		  AND CASE WHEN rc.LV_Client_Name='veldos' THEN rc.service_name ELSE '' end 
		         NOT IN('Payment Verification Manual')
		  --12/8/21 - TedM TedM request no limits on Amex for call_center_name=Non Reg F
		  AND CASE WHEN rc.LV_Client_Name='veldos' THEN rc.call_center_name ELSE '' end 
		         NOT IN('Non Reg F')
		  --11/29/21 - TedM adds LiveVox_Result suppressions for RGS-CCS
		  --11/30/21 - TedM adds:  'Busy','Fax','Invalid Phone Number'
		  --1/3/22 - Brett adds:  'AGENT - CUST RPC PTP 6'
		  AND CASE WHEN rc.LV_Client_Name='RGS-CCS' THEN rc.Livevox_Result ELSE '' end 
		  ...
		  --11/12/21 - TedM requests not evaluating customerid=0 and dialedphone not 800 number (transfers)
		  AND (CASE WHEN LEFT(fct.DialedPhoneNumber,3)='800' AND cust.CustomerId=0 THEN 1 ELSE 0 END) = 0
		  --11/29/21 - TedM adds Service_Name suppressions for RGS-CCS
          --9/8/22 - Brett adds services to suppress
		  AND rc.service_name NOT IN('DEC_1P-DCA CTD','DEC_1P-DCA HCI','DEC_1P-DCA Inbound','DEC_1P-DCA Manual','DEC_1P-DCA QC',
									 ...
									 'LXA_3P-ACB_DSA_CTD','LXA_3P-ACB_DSA_Inbound','LXA_3P-TD_DSA_CTD','LXA_3P-TD_DSA_Inbound')
          --7/5/22  -  TedM requests all services containing "HTI" (sms) to be suppressed 
		  and rc.service_name not like '%HTI%'
		  --12/10/21 - Brett adds clientids for RGS-CCS
		  --1/24/22	 - Brett adds SNBCE1
	      --6/16/22 - ATTMOB suppressed per Ted
	      ...
	      --all clients 11/30/21+
		  AND (
		       dt.CalendarDate>='11/30/21'
          --12/13 - BobR requests removal of testing data prior to 11/30/21 so need to stop pulling the below into calls to eval
		  /*
			   OR
		  --subsets prior to 12/1/21
  		        (
  			      (
				   dt.CalendarDate>='5/1/21'
	               AND rc.Service_Id IN(133282,133280,133281,133283,133284,133709,133711,133708,133710,133712,134677,134678,134679,
                                        134680,134685,134686,134687,134688,134673,134674,134675,134676,134681,134682,134683,134684,
							            90277,90278,90279,90281,90282,130169,130170,90283)
				  )
				  OR
                  (
				   dt.CalendarDate>='11/1/21'
				   AND dcl.sourcesystem='AMEX Latitude'
				   AND dcl.clientid NOT LIKE 'RL%'
				  )
			    )
		   */
			  )
